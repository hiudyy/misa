...
The single source is `dados/config.json`. Current `schemaVersion` is 1. Unversioned files migrate automatically; malformed JSON is preserved as `.corrupt-*`. A future schema stops startup without overwriting data. Operational changes require a restart.
