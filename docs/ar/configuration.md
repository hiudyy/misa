...
المصدر الوحيد هو `dados/config.json`. قيمة `schemaVersion` الحالية 1. تُرحّل الملفات بلا نسخة؛ ويحفظ JSON التالف باسم `.corrupt-*`. يوقف schema أحدث التشغيل دون الكتابة فوق البيانات. تتطلب تغييرات التشغيل restart.
